"""
Training script for WARBERT(M) - Match-type pairwise mashup-API matching.
"""
import os
...
class MashupAPIPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mashup_text = f"{mashup['mashup_name']}. {mashup['mashup_desc']}"
        api_text = f"{api['api_name']}. {api['api_desc']}"
...
def evaluate_m(model, eval_data, api_corpus, tokenizer, device, config):
    ...
    api_texts = [f"{api['api_name']}. {api['api_desc']}" for api in api_corpus]

    all_scores, all_labels = [], []

    for mashup_entry in tqdm(eval_data, desc="Evaluating"):
        mashup_text = f"{mashup_entry['mashup_name']}. {mashup_entry['mashup_desc']}"
...
def main():
    ...
    total_steps  = len(train_loader) * config.NUM_EPOCHS_M
    warmup_steps = len(train_loader) * config.WARMUP_EPOCHS_M
    ...
    for epoch in range(config.NUM_EPOCHS_M):
        ...
        train_loss, peak_mem, epoch_time = train_epoch(model, train_loader, optimizer, scheduler, device, config)
